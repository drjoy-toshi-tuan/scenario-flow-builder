...
const DICT: Record<Lang, Record<string, string>> = {
  vi: {
    ...
    aiGenerate: 'AIで生成・修正',
    aiContextSection: 'Bối cảnh gửi AI (không sửa được)',
    aiRoleScript:
      'AI đóng vai kỹ sư IVR Brekeke: sinh/sửa script JavaScript phân tích câu trả lời của người gọi cho câu hỏi bên dưới. AI chỉ sửa đúng phần bạn yêu cầu.',
    aiRolePrompt:
      'AI đóng vai chuyên gia prompt: sinh/sửa prompt cho node OpenAI để phân tích câu trả lời của người gọi cho câu hỏi bên dưới. AI chỉ sửa đúng phần bạn yêu cầu.',
    aiQuestionNode: 'Node câu hỏi (Announce / Interaction)',
    aiQuestionNodeNone: '— Không dùng node câu hỏi —',
    aiAnnounceLabel: 'Câu hỏi (announce) — tự lấy từ node đã chọn',
    aiNoAnnounce: '(node chưa có nội dung announce)',
    aiAttachScriptNew: 'Toàn bộ YAML của flow sẽ được gửi kèm làm ngữ cảnh chung.',
    aiAttachScriptEdit:
      'Toàn bộ YAML của flow + script hiện tại sẽ được gửi kèm — AI hiểu là cần CHỈNH SỬA script này.',
    aiAttachPromptNew: 'Toàn bộ YAML của flow sẽ được gửi kèm làm ngữ cảnh chung.',
    aiAttachPromptEdit:
      'Toàn bộ YAML của flow + prompt hiện tại sẽ được gửi kèm — AI hiểu là cần CHỈNH SỬA prompt này.',
    aiUserPrompt: 'Yêu cầu của bạn',
    aiUserPromptPh: 'Mô tả nội dung cần AI tạo mới hoặc chỉnh sửa…',
    aiGenerateBtn: 'Generate（生成）',
    ...
    aiRegenerate: '再生成',
    aiExplainEmpty: 'Chưa có giải thích. Bấm 再生成 để AI đọc code và giải nghĩa.',
    // Nhánh
  ...
  ja: {
    ...
    aiGenerate: 'AIで生成・修正',
    aiContextSection: 'AIに送るコンテキスト（編集不可）',
    aiRoleScript:
      'AIはBrekeke IVRエンジニアとして、下記の質問に対する発信者の回答を解析するJavaScriptスクリプトを生成・修正します。指示された部分のみ修正します。',
    aiRolePrompt:
      'AIはプロンプトエンジニアとして、下記の質問に対する発信者の回答を解析するOpenAIノード用プロンプトを生成・修正します。指示された部分のみ修正します。',
    aiQuestionNode: '質問ノード（アナウンス／インタラクション）',
    aiQuestionNodeNone: '— 質問ノードを使わない —',
    aiAnnounceLabel: '質問アナウンス — 選択ノードから自動取得',
    aiNoAnnounce: '（アナウンス内容がありません）',
    aiAttachScriptNew: 'フロー全体のYAMLもコンテキストとして送信されます。',
    aiAttachScriptEdit:
      'フロー全体のYAMLと現在のスクリプトも送信されます — AIはこのスクリプトの修正として扱います。',
    aiAttachPromptNew: 'フロー全体のYAMLもコンテキストとして送信されます。',
    aiAttachPromptEdit:
      'フロー全体のYAMLと現在のプロンプトも送信されます — AIはこのプロンプトの修正として扱います。',
    aiUserPrompt: '指示内容',
