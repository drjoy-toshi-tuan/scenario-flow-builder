...
- 📤 **YAML エクスポート**（IR ↔ YAML のラウンドトリップ）と 💾 Drive API 経由の **Google Drive へ直接保存**。
- 🔐 Google ログイン、`@drjoy.jp` アカウントのみ許可 — クレーム検証＋nonce を厳格に実施（[セキュリティ](#-セキュリティ)参照）。
- 📁 **Google Drive 上のフロー管理**：`病院/シナリオ/<シナリオ名>_V{N}.yaml` の 3 階層でバージョン管理（作成／インポート／複製／メモ／削除）。
...
ログイン後、**フロー管理**画面（[§フロー管理](#-フロー管理google-drive)参照）でバージョンを選択してキャンバスに開きます。
...
> （フローの読み書きには Google Drive への接続（同意ポップアップ）が必要、AI 機能には `VITE_AI_PROXY_URL` が必要です。）
...
npm test           # vitest run — fromYaml/toYaml, verifyIdToken, icon などの単体テスト
...
| `VITE_DRIVE_ROOT_FOLDER_ID` | – | フローを格納する Google Drive のルートフォルダ ID（既定は [`src/drive/config.ts`](src/drive/config.ts)）。 |
...
ログイン後、キャンバスに入る前に **フロー管理** 画面が開きます。フローは Google Drive の
共有フォルダに 3 階層 `病院/ シナリオ/ <シナリオ名>_V{N}.yaml` で保存されます:

- 📂 **病院 › シナリオ › バージョン** のドリルダウン一覧（検索・ソート・ページング付き）。
- ✨ **新規フロー作成** / 📤 **YAML インポート** — 保存先の病院・シナリオを選択（または新規作成）し、`V{max+1}` として保存。
- 📄 バージョン行のホバーアクション: **メモ**（ファイルの description に保存、ホバーで内容をプレビュー）・**複製**（＝旧バージョンの復元にも使用）・**削除**（Drive のゴミ箱へ、~30 日は復元可能）。
- 💾 キャンバス内: **保存** で開いているバージョンのファイルを上書き（`更新日時` が自動更新）。

### アクセスと権限

- Drive へは ユーザー本人の **OAuth アクセストークン**で直接アクセスします。同意ポップアップは
  **各アカウント初回のみ** — 以降はトークン（寿命 ~1 時間、`localStorage`）を
  `DriveTokenKeeper` がバックグラウンドで自動更新します。バンドルに秘密情報は含めません。
- アプリ権限（owner / admin / user）は Drive 上の `access-log.json` に保存: アクセスした
  アカウントを自動記録し、owner が「権限管理」モーダルで Admin/User を切り替えます。
  病院／シナリオ／バージョンの削除は owner / admin のみ（バックエンドがないため UX ゲート
  であり、絶対的な防壁ではありません）。
...
drive/      # Google Drive REST v3 (純粋 fetch): api, config, token, useDriveAuth, permissions, errors
files/      # DriveManagerScreen, DriveConnectPanel, PermissionsModal (キャンバス前のフロー管理画面)
...
- 📤 **Export YAML** (round-trip IR ↔ YAML) và 💾 **lưu thẳng lên Google Drive** qua Drive API.
- 🔐 Đăng nhập Google, chỉ tài khoản `@drjoy.jp` — verify claim kỹ + nonce (xem [Bảo mật](#-bảo-mật)).
- 📁 **Quản lý flow trên Google Drive**: cây 3 tầng `Bệnh viện/Kịch bản/<Tên>_V{N}.yaml` theo version (tạo / import / duplicate / ghi chú / xoá).
...
Sau khi đăng nhập, chọn phiên bản từ màn **Quản lý flow** (xem [§Quản lý flow](#-quản-lý-flow-google-drive))
để mở trên canvas.
...
> (Đọc/ghi flow cần kết nối Google Drive — popup đồng ý 1 lần; dùng AI cần `VITE_AI_PROXY_URL`.)
...
npm test           # vitest run — unit test cho fromYaml/toYaml, verifyIdToken, icon…
...
| `VITE_DRIVE_ROOT_FOLDER_ID` | – | ID folder gốc trên Google Drive chứa flow (mặc định trong [`src/drive/config.ts`](src/drive/config.ts)). |
...
Sau khi đăng nhập, app mở màn **Quản lý flow** trước khi vào canvas. Flow lưu trên folder
chia sẻ Google Drive theo cây 3 tầng `Bệnh viện/ Kịch bản/ <Tên kịch bản>_V{N}.yaml`:

- 📂 Danh sách drill-down **Bệnh viện › Kịch bản › Phiên bản** (kèm tìm kiếm, sort, phân trang).
- ✨ **Tạo flow mới** / 📤 **Import YAML** — chọn (hoặc tạo mới) bệnh viện + kịch bản đích, lưu thành `V{max+1}`.
- 📄 Hover lên dòng phiên bản để thao tác: **Ghi chú** (lưu vào description của file, hover xem nhanh
  nội dung), **Duplicate** (= cách khôi phục bản cũ), **Xoá** (đưa vào Thùng rác Drive, ~30 ngày khôi phục được).
- 💾 Trong canvas: nút **Lưu** ghi đè file phiên bản đang mở (`更新日時` tự nhảy).

### Truy cập & phân quyền

- App gọi thẳng Drive API bằng **access token OAuth của chính người dùng**. Popup đồng ý chỉ hiện
  **1 lần cho mỗi tài khoản** — về sau token (sống ~1 giờ, giữ trong `localStorage`) được
  `DriveTokenKeeper` tự gia hạn chạy nền. Không có secret nào trong bundle.
- Phân quyền app (owner / admin / user) lưu trên Drive trong file `access-log.json`: app tự ghi
  nhận tài khoản đã truy cập; owner gạt Admin/User trong modal **Quản lý quyền**. Chỉ owner/admin
  mới xoá được bệnh viện / kịch bản / phiên bản (đây là cổng UX — app không backend nên không
  phải hàng rào bảo mật tuyệt đối).
...
canvas/     # React Flow — FlowCanvas, irAdapter, nodes/, edges/
auth/       # AuthProvider, useAuth, config (ALLOWED_DOMAIN), LoginScreen, verifyIdToken, nonce
drive/      # Google Drive REST v3 (thuần fetch): api, config, token, useDriveAuth, permissions, errors
files/      # DriveManagerScreen, DriveConnectPanel, PermissionsModal (màn quản lý flow trước canvas)
