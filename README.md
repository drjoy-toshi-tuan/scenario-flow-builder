...
- Drive へは ユーザー本人の **OAuth アクセストークン**で直接アクセスします（初回のみ同意
  ポップアップ、トークンは `sessionStorage`）。バンドルに秘密情報は含めません。
...
- App gọi thẳng Drive API bằng **access token OAuth của chính người dùng** (popup đồng ý 1 lần,
  token giữ trong `sessionStorage`). Không có secret nào trong bundle.
